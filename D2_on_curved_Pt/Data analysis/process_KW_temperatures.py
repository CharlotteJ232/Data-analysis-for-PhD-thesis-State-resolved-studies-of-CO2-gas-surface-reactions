...
def main():
    ...
    for temp in temperatures:
        ...
        for position in positions:
            ...
            errordic[position].append(measurements[temp][position].sigma)
    ...
    for position in positions:
        ...
        errordic[position] = np.array(errordic[position])
        # plt.plot(temperatures, stickingdic[position],'o-',c=color,label=str(position)+' mm')
        plt.errorbar(temperatures, stickingdic[position], yerr=errordic[position],capsize=10,c=color,label=str(position)+' mm')
    ...
    #Save data and error to text file. column 1: temperature, column 2: position, column 3: data, column 4: error
    savearray = []
    for pos in positions:
        for temp, index in zip(temperatures, range(len(temperatures))):
            savearray.append([temp, pos, stickingdic[pos][index],errordic[pos][index]])
    savearray = np.array(savearray)
    np.savetxt(folder+'Images/s0_pos_temp.txt',savearray,header='Temperature, position, data, error')
...
class measurement:
    ...
    def plot_measurement_set(self, save=False):
        for name in self.datadict.keys():
            ...
            plt.plot(self.timedict[name],self.datadict[name],color='0.5', linewidth=1) #str(measurement_number/np.max(datasets)*0.7)
