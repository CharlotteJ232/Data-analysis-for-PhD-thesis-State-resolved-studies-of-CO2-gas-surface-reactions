...
def main():
    ...
    plt.legend()
    plt.savefig(savefolder+'all_s0')
    plt.show()
    plt.close()     
    
    
    center = fit_all_slopes(datalist,fitrange=fitrange,center_guess=20, typ='line')
...
#        print (center)
    ...
    def cos_prediction(self, center, slopeleft, sloperight, offset, left=None, right=None):
#        print (center)
        positions = self.pos-center   
        angles = np.arcsin(positions/15)
        fitleft = slopeleft * np.cos(angles[left])*positions[left] + offset
        fitright = sloperight * np.cos(angles[right])*positions[right] + offset
        
        return np.concatenate((fitleft,fitright))
    
    def cos2_prediction(self, center, slopeleft, sloperight, offset, left=None, right=None):
#        print (center)
        positions = self.pos-center 
        angles = np.arcsin(positions/15)
        angles = np.arcsin(positions/15) #15 mm is the radius of the crystal cylinder
        fitleft = slopeleft * np.cos(angles[left])**2*positions[left] + offset
        fitright = sloperight * np.cos(angles[right])**2*positions[right] + offset
        
        return np.concatenate((fitleft,fitright))

def fit_all_slopes(datalist, fitrange=2.0, center_guess=20, center_bounds=(19.91, 20.99), typ='line'):
    """
    typ can be 'line', 'cos' and 'cos2'
    """
...
#        print(fitparams[0])
        for dataset,i in zip(datalist,range(0,3*len(datalist),3)):
            ...
            positions = dataset.pos-center_guess #dataset.pos-fitparams[0]
            left = positions<0
            ...
            if typ == 'line':
                res = dataset.linear_prediction(fitparams[0],*fitparams[i+1:i+4], left=left, right=right)
            elif typ == 'cos':
                res = dataset.cos_prediction(fitparams[0],*fitparams[i+1:i+4], left=left, right=right)
            elif typ == 'cos2':
                res = dataset.cos2_prediction(fitparams[0],*fitparams[i+1:i+4], left=left, right=right)
            else:
                print ('select "line", "cos" or "cos2" as fit type')
                

            res_total = np.concatenate((res_total, res))
        assert len(res_total) == len(concatenated_data)
        return res_total - concatenated_data   
...
#    print (fitparams,'\n')
    ...
    lower[0]=center_bounds[0]
    upper[0]=center_bounds[1]
    bounds = (lower, upper)
    print (bounds)
    
    #THE FIT 
    print('fitparameters', fitparams)
...
#    print(result.x)
    ...
    for dataset,i in zip(datalist, range(0,3*len(datalist),3)):
        center = result.x[0]
        positions = dataset.pos-center_guess #convert positions so center is at x=0
        angles = np.arcsin(positions/15)
        ...
        if typ=='line':
            ...
            dataleft = result.x[i+1]*positions[left]*np.cos(angles[left])+result.x[i+3]
            dataright = result.x[i+2]*positions[right]*np.cos(angles[right])+result.x[i+3]     
        elif typ=='cos2':
            dataleft = result.x[i+1]*positions[left]*np.cos(angles[left])**2+result.x[i+3]
            dataright = result.x[i+2]*positions[right]*np.cos(angles[right])**2+result.x[i+3]               
        if save_fit:
...
def plot_energy_test_convolution(datalist):
    ...
    plt.plot(np.arange(0.7,6.1,0.01), np.exp(-np.arange(0.7,6.1,0.01)),c='black', label='Function without convolution')
