...
def main():
    ...
    center = fit_all_slopes(datalist,fitrange=fitrange,center_guess=19.95)
...
class data:
    ...
    def convert_step_density(self, center,save=False, plot=False):
        ...
        Assumes left side is B steps, right side is A steps
        """
        step_height = 0.2265637926 #nm, for steps on pt111
        radius = 0.13874 #nm, for platinum
        A_length = np.sqrt(3)*radius
        B_length = np.sqrt(3)*radius
        ...
        step_fraction_A = A_length*self.probed_step_density[self.pos-center<0]
        step_fraction_B = B_length*self.probed_step_density[self.pos-center>=0]
        
        self.step_fraction = np.absolute(np.concatenate((step_fraction_A,step_fraction_B)))
    ...
    def linear_prediction(self, center, B_step, A_step, terrace, left=None, right=None):
        """
        This is the fitfunction for a simple line with a slope and offset
        """
        self.convert_step_density(center)
        positions = self.pos-center        
        fitleft = B_step  * self.step_fraction[left] + terrace * (1-self.step_fraction[left]) 
        fitright = A_step * self.step_fraction[right] + terrace * (1-self.step_fraction[right]) 
...
def fit_all_slopes(datalist, fitrange=2.0, center_guess=20):
    ...
    def residuals(fitparams):
        ...
        fitrange does not work for all values (something about array length)
        """
        res = np.array([])
    ...
    for dataset,i in zip(datalist, range(0,3*len(datalist),3)):
        # plt.plot(dataset.pos, dataset.step_fraction, dataset.pos, 1-dataset.step_fraction)
        center = result.x[0] 
        dataset.convert_step_density(center)
        # plt.plot(dataset.pos, dataset.step_fraction, dataset.pos, 1-dataset.step_fraction)
        # print(dataset.step_fraction, 1-dataset.step_fraction)
        ...
        #dataleft and dataright are the fitted lines, not actual data
        dataleft = result.x[i+1]*dataset.step_fraction[left]+result.x[i+3]*(1-dataset.step_fraction[left])
        dataright = result.x[i+2]*dataset.step_fraction[right]+result.x[i+3]*(1-dataset.step_fraction[right])
