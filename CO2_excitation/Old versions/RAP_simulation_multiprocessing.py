...
#             'R10':[0.261387, 0.260304, 0.25703, 0.251478, 0.243492, 0.232823, 0.219079, 0.201631, 0.179402, 0.150287, 0.108893]} #repeat 0.179402

        
cg_lists = {'Test':[0.5, 0.75, 1],
            'R0': [1],
            'R2': [0.774597 ,0.730297,0.57735],
            'R4': [np.average(np.array([0.745356, 0.730297, 0.68313, 0.596285, 0.447214]))]}
...
folder = folderstart+'Laser/Simulations/CO2/220530/'
...
rx_list=[0.003]       # radius of unfocused laser in x (m)
...
nmax=500  # number of trajectories for each laser power step, was 500
...
for transition in transitions:
    lam = wl[transition]
    A21 = 2/np.pi * A21_dic[transition]
    cg_list = cg_lists[transition]
    savefolder = folder+transition+'/'
    if save:
