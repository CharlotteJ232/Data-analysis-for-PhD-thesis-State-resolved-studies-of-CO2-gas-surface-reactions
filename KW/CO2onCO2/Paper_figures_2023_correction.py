...
folder = 'DATA/2023/05 May/230508/KW/'
savefolder = 'Proefschrift/CO2_on_CO2_physisorption/Figures/Corrected/'
# kwname = 'KW03' #old
kwname = 'KW03'
lasername = 'onresonance_R4_laser'
filename_normalize = 'KW_test2'
normalize_guess = 3E-10 #make False if not in use
folder_normalize = 'DATA/2023/06 Jun/230622/Pressure/'
start = 200
# start = 3000
# start = 2000
start = 1
...
plotlines = [(21, 'open flag 1'), (31, 'open flag 2'), (110, 'close flag 2'), (120.5, 'close flag 1')]
...
def main():
    ...
    time_normalize, data_normalize = np.loadtxt(folderstart+folder_normalize+filename_normalize+'.txt', skiprows=0, usecols=(0,1), unpack=True)
    ...
    print('S (begin and end) ', S0)

    if normalize_guess:
        norm = normalize_guess
    ...
    #FFT plot
    fft_plot(xmin=2.986, xmax=3.014, save=save_figures, norm=norm)
...
def kw_plot(time, data, axis=[None,None,None,None], start=None, stop=None, save=False, plotlines=[], plotmasks=None):
    ...
    if save:
        ...
        plt.savefig(folderstart+savefolder+'KW_plot.pdf', bbox_inches='tight', dpi=500)
...
def fft_plot(xmin=None,xmax=None, save=False, norm=1, freq=3.0, freq_tolerance=1E-4):
    ...
    #or simply this
    center = (fftfreq_laser > freq-freq_tolerance) * (fftfreq_laser < freq+freq_tolerance)
    print('this is the maximum change in sticking probability: ', fft[center]/fft_laser[center])
...
def mask_plot(time, time_laser, modulation, data, axis=[None,None,None,None], save=False):
    ...
    if save:
        ...
        plt.savefig(folderstart+savefolder+'mask_plot.pdf', dpi=500)
    plt.show()
    plt.close()
