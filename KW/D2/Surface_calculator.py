...
position = 1.9 #mm, position that you want to use for calculations
...
def main():    
    positions, step_densities = convert_step_density(positions_array)
    
    step_density = np.interp(position, positions, step_densities)
    print ('step density at angle '+str(angle)+' degrees and position '+str(position)+' mm is: '+str(step_density))
    
    terrace_fraction(positions, step_densities, plot=True)
    
    unit_cell_length = 1/np.absolute(step_density)/row_width
    print("unit cell length ",unit_cell_length)
...
def convert_step_density(positions, plot=True):
    ...
    step_density = step_density(positions, step_height)
